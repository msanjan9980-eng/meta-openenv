...
def print_claim(claim_data: dict) -> None:
    ...
    print()
    uw = claim_data.get("underwriting_signals") or {}
    if uw:
        print("UNDERWRITING SIGNALS (indirect scores):")
        for k, v in uw.items():
            print(f"  {k}: {v:.3f}")
        print()
...
def play_episode(difficulty: str) -> None:
    ...
    while not done:
        ...
        print(f"\nREWARD FOR THIS STEP: {reward:.3f}  (strictly between 0 and 1)")
        print(f"DONE: {done}")
        ...
        if done:
            ...
            print(f"  Episode return : {reward:.3f}  (aggregated score on last step)")
            fc = info.get("final_components") or {}
            if fc:
                print(f"\nCOMPONENT BREAKDOWN (canonical, pre-mapping):")
                print(f"  Decision (weighted) : {fc.get('decision', 0):.3f}")
                print(f"  Fraud (weighted)    : {fc.get('fraud_detection', 0):.3f}")
                print(f"  Reasoning (weighted): {fc.get('reasoning', 0):.3f}")
                print(f"  Efficiency (weighted): {fc.get('efficiency', 0):.3f}")
                print(f"  Sequence readiness  : {fc.get('sequence_readiness', 0):.3f}")
            print_separator()
